...
export default class extends Controller {
    ...
    handleDragStart(event){
        ...
        const item = event.target.closest('[data-xorgxx--neox-dashboard-bundle--neox-drag-tabs-target="tab"]');
        if(item){
            event.dataTransfer.setData("text/plain", item.dataset.id);   // URL associée
    ...
    handleDragOver(event){
        ...
        const targetElement = event.target.closest('[data-xorgxx--neox-dashboard-bundle--neox-drag-tabs-target="tab"]');
        if(targetElement){
            targetElement.classList.add('drag-hover'); // Add hover effect during drag on the child
        }
    }
    ...
    handleDragLeave(event){
        const targetElement = event.target.closest('[data-xorgxx--neox-dashboard-bundle--neox-drag-tabs-target="tab"]');
        if(targetElement){
            targetElement.classList.remove('drag-hover'); // Remove hover effect during drag on the child
        }
    }
    ...
    handleDrop(event){
        ...
        const draggedElement = document.getElementById("neoxTab-" + draggedId);
        const targetElement = event.target.closest('[data-xorgxx--neox-dashboard-bundle--neox-drag-tabs-target="tab"]');
